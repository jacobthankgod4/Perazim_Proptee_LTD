...
$stmt = $pdo->prepare('SELECT p."Id", p."Title", p."Type", p."Address", p."City", p."State", p."Zip_Code", p."Images", p."Video", i."Id_in", i."interest", i."share_cost", i."expected_inv", i."current_inv" FROM public.property p LEFT JOIN public.investment i ON p."Id"=i.property_id WHERE i.property_id = :id ORDER BY i."Id_in" ASC');
$stmt->execute([':id' => $id]);
...
foreach ($r51 as $fetch_array) {  
    ...
    $fetch_array_id_in[$count_fetch_proptee]=$fetch_array['Id_in'];
...
$fetch_array_generated[$count_fetch_proptee]=round($fetch_array_current_inv[$count_fetch_proptee]/1000000);
    ...
    if ($fetch_array_share_cost[$count_fetch_proptee] <= 500000) {
            ...
            $fetch_array_interest[$count_fetch_proptee]=9.25;
        
    } elseif ($fetch_array_share_cost[$count_fetch_proptee] >= 500000) {
        
            $fetch_array_interest[$count_fetch_proptee]=8.8;
