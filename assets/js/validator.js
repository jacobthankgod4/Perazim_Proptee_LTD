...
console.log('sss2')
	...
	if (pass11==="") {
		...
		validate__error(pass1, 'Password must contain an uppercase and a lowercase letter, a number, a special character and must be more then 8');
    ...
    if (first=="true" && second=="true" && fourth=="true"  && fifth=="true"  && sixth=="true"&& seventh=="true"  && eigth=="true"  && ninth=="true") {

		console.log("hellojjjj");

		let xhr = new XMLHttpRequest();
		xhr.open("POST", "includes/auth//reg.data.config.php", true);
		xhr.onload = () => {
				...
				if (xhr.status === 200) {
					...
					if (stab=document.querySelector(".hi2")) {
						Swal.fire({
							icon: 'success',
							title: 'Hurray',
							text: 'you have successfully registered...check your email to validate your account. NB: please check your Spam folder incase you did not find it in your regular mailbox',  
							showClass: {
